...
An autonomous, always-on AI personal finance sentinel inspired by the **OpenClaw agent architecture**. It links bank accounts via Plaid, uses **CockroachDB v25.4** as its long-term persistent memory with **Distributed Vector Indexing (768-dim embeddings)**, leverages the **CockroachDB Managed MCP Server** for open-ended financial reasoning, runs via an autonomous **24/7 Heartbeat Daemon on AWS**, and proactively alerts & interacts with the user in real-time on **Telegram** (`@KadmusFinanceBot`).
